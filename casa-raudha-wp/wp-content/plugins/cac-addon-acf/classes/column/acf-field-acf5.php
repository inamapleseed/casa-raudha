...
class CPAC_ACF_Column_ACF_Field_ACF5 extends CPAC_ACF_Column_ACF_Field {
	...
	public function display_settings() {
		...
		<tr class="column_field">
			<?php $this->label_view( __( 'Field', 'cpac' ), __( 'Select your ACF field.', 'cpac' ), 'field' ); ?>
			<td class="input">
						...
						<?php foreach ( $optiongroups as $group ) : ?>
							<optgroup label="<?php echo esc_attr( $group['title'] ); ?>">
								<?php foreach ( $group['options'] as $field_key => $option ) : ?>
									<option data-field-type="<?php echo esc_attr( $option['type'] ); ?>" value="<?php echo $field_key ?>"<?php selected( $field_key, $this->get_field_key() ) ?>><?php echo $option['label'] ? $option['label'] : __( 'empty label', 'cpac' ); ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endforeach; ?>
					</select>
				<?php else : ?>
					<?php _e( 'No ACF fields available.', 'cpac' ); ?>
				<?php endif; ?>

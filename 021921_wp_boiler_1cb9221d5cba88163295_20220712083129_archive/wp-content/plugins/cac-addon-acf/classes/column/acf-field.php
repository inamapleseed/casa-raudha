...
abstract class CPAC_ACF_Column_ACF_Field extends CPAC_Column {
	...
	public function init() {
		...
		$this->properties['label']			= __( 'Advanced Custom Fields', 'cpac' );
		$this->properties['is_cloneable']	= true;
		$this->properties['group']			= 'integration';
		...
		switch ( $field['type'] ) {
			...
			case 'file':
				if ( $value ) {
					...
					if ( ! $attachment ) {
						$value = '<em>' . __( 'Invalid attachment', 'cpac' ) . '</em>';
					}
					else {
			...
			case 'repeater':
				...
				if ( ! empty( $field['sub_fields'] ) ) {
					...
					if ( $field_value = acf_get_value( $this->get_formatted_id( $id ), $field ) ) {
						...
						foreach ( $field_value as $values ) {
							...
							foreach ( $values as $field_key => $v ) {
								...
								// Single selected sub_field
								if ( ! empty( $this->options->sub_field ) ) {
									if ( $this->options->sub_field === $field_key ) {
										$output[] = '<div class="sub_field">' . $this->format_acf_value( $v, $sub_fields[ $field_key ], $id ) . '</div>';
									}
								}

								// Display all sub_fields
								else {
									$output[] = $this->format_acf_value( $v, $sub_fields[ $field_key ], $id );
								}
	...
	public function display_field_sub_field_picker() {
		...
		<tr class="column-sub_field">
			<?php $this->label_view( __( 'Sub Field', 'cpac' ), __( 'Select a repeater sub field.', 'cpac' ), 'sub_field' ); ?>
			<td class="input">
			<?php if ( $sub_fields ) : ?>
			...
			<?php else : ?>
				<?php _e( 'No ACF subfields available.', 'cpac' ); ?>
			<?php endif; ?>
			</td>
		</tr>
		<?php

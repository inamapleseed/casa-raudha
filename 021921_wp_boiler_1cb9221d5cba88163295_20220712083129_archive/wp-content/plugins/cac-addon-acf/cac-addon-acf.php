...
class CPAC_Addon_ACF {
	...
	public function column_type_groups( $groups ) {
		...
		$groups['integration'] = __( 'Plugins', 'cpac' );
	...
	function display_plugin_row_notices() {
		...
		if ( ! $this->is_acf_active() ) {
			$missing_dependencies[] = '<a href="' . admin_url( 'plugin-install.php' ) . '?tab=search&s=Advanced+Custom+Fields&plugin-search-input=Search+Plugins' . '" target="_blank">' . __( 'Advanced Custom Fields', 'cpac' ) . '</a>';
		}
			...
			<tr class="plugin-update-tr">
				<td colspan="3" class="plugin-update">
					<div class="update-message">
						<?php printf( __( 'The ACF add-on is enabled but not effective. It requires %s in order to work.', 'cpac' ), $missing_list ); ?>
					</div>
				</td>
			</tr>
			<?php
